...
$db = new Database();
$db->connect();
$db->select('event_service', '*', 'event', 'event_service.service_id = ' . $item[$i]->getId() . ' AND event_service.status != "canceled" AND event_service.event_id = event.id AND event.status != "canceled"', null, null);
$res = $db->getResult();
...
if (count($res) == 0) {
  $reservations = '<tr><td style="text-align: center;" colspan="2"><label>Nincs lefoglalt időpont!</label></td></tr>';
}
...
<form method="POST">
  ...
  if (count($res) != 0) {
    ...
    <input type="hidden" id="action" name="action" value="event_service_insert"/>
    <input type="hidden" id="company_id" name="company_id" value="' . $item[$i]->getOwnerId() . '"/>
    <input type="hidden" id="service_id" name="item_id" value="' . $item[$i]->getId() . '"/>
    <div class="row">
